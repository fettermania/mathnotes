...
\begin{itemize}
\item Gauss's Law says $\iint_S \vec{E} \cdot \vec{dA} = \frac{Q_{tot}}{\epsilon_0}$ 
\item Combinig this with the divergence theorem), we know \fbox{$\nabla \cdot \vec{E} = \frac{\rho}{\epsilon_0}$} for the charge density $\rho(\vec{x})$.
...
\item Experimental equation for electric field and magnetic flux: $\int_C \vec{E} \cdot \vec{dA} = -\frac{d \Phi}{dt}$
...
\item while $Q = \iiint_V  \rho d\vec{x} \Rightarrow  \frac{\delta Q}{\delta t}  =  \frac{\delta}{\delta t} \iiint_V \rho d\vec{x}= \iiint_V  \frac{\delta \rho}{\delta t} $\
