...
\textbf{Energy}: U is a position-consuming function, an example of a potential. Energy $U$ for charge amount $q$ in field $\vec{E}$ is \fbox{$-q\vec{E} = \nabla U$}.
\\
...
\fbox{\textbf{Work} is change in energy, e.g. $W_{field} = -[U(\vec{x_f} )- U(\vec{x_0})]$. } If the charge flows with the field, then the field is doing positive work.  If the charge flows against the field, the field is doing negative work.  So in the above case, $W_{field} = -[U(x_f,0,0) )- U(x_0, 0, 0)]  \Rightarrow -[-qE_0x_f - -qE_0x_0 = qE_0(x_f - x_0)$.
...
\fbox{So we can take a \textbf{line integral} to measure work over a path in a field.}
...
Example: Infinite wire of current going up on the z-axis.  Field equation, for (generated?) field $\vec{B}$, penetrated region bounded by curve $C$, unit tangent (to the curve) $\hat{T}$ , constant $\mu_0$ and current amount $I$: \fbox{$\int_C [\vec{B} \cdot \hat{T}]ds = \mu_0I$}.  What is $\| \vec{B} \|$ at some distance from the z-axis $r$?
\begin{itemize} 
...
\begin{itemize}
\item $\Lambda^2(\mathbb{R}^3)^*$ often substitutes names: $dx \leftrightarrow dx_1, dy \leftrightarrow dx_2, dz \leftrightarrow dx_3$
...
Summary: Multivariable Gaussian integrals are quadratic combos of $x, y, z,$ etc. and \fbox{$\int_{\mathbb{R}^2} e^{-\frac{1}{2} [\vec{x} \cdot (A\vec{x})]} = \sqrt{\frac{(2\pi)^n}{\det(A)}}$}.  To evaluate them, you can split them out Fubini-style and multiply, you can get to a diagonal matrix, or you can rotate to a diagonal and back, which has the same determinant.
...
Summary: By employing the Dirac Delta function \fbox{$\delta(\vec{x}) \approx \int_{\mathbb{R}^n} e^{2\pi i \vec{x}\cdot\vec{k} }d\vec{k}$}, we can transform functions into a separate eigenspace where derivatives and integrals may be easier.
...
\\
 \fbox{any function $f(\vec{x}) = \int_{\mathbb{R}^n} \hat{f}(\vec{k})e^{2\pi i \vec{k}\cdot \vec{x}} d\vec{k}$ for some function $\hat{f}_{\vec{k}}$ called the \textbf{Fourier Transform} of $f$}.
...
\item Also, \fbox{Fick's Law: $\vec{J} = -D\nabla \rho$}, since the rate at which we're leaving an area is the negative of the gradient (which points towards the quantity increasing).
...
\item Therefore, we can use the standard $f' = kf$, and the knowledge that $t=0 \rightarrow \hat{u} = \hat{u}(\vec{k}, 0)$ to find \textbf{the solved diff eq on the Fourier side} \fbox{ $\hat{u}(\vec{k}, t) = \hat{u}(\vec{k}, 0)e^{ -4\pi^2\|k\|^2t}$} if we assume $D=1$
\end{itemize}
...
\item Leading us to \fbox{$u(\vec{x}, t)= (\frac{1}{4 \pi D t})^{\frac{3}{2}} \iiint u(\vec{y}, 0) e^{-\frac{1}{4 Dt} \| x - y\|^2}d\vec{y}$} as the almost full solution.  Just need the initial constant.
...
Summary: General wave equation form: \fbox{$ \frac{\delta^2u}{\delta t^2}  = v^2 \nabla^2u$} 
...
\item With ``wave speed'' $v$, for some reason $v^2 = \frac{kl^2}{m}$, for \textbf{general wave equation form} \fbox{$ \frac{\delta^2u}{\delta t^2}  = v^2 \nabla^2u$} 
...
If \fbox{$\hat{u}(k,t) = \hat{u}(k,t) e^{2\pi i k l}$}, then 
\begin{align}
...
\item Set $A_m = 2ia_m(t) \Rightarrow$ \fbox{$u(x, t)  = A_m(t)[\sin(2\pi i x \cdot \frac{m}{l})]$}
\item Repeating this in 2D yields a \textbf{Fourier sine series} 
\fbox{$u(x,y,t) = \sum_{m,n = 1}^{\infty}A_{mn}(t)\sin(\frac{2\pi m}{l_x} x\sin(\frac{2\pi m}{l_y} y)$}
\end{itemize}
...
With an electric field $\vec{E}$ and a related (generated?) magnetic field $\vec{B}$, Helmholtz has a theorem: \fbox{As long as the fields go to $\vec{0}$ at infinity, the divergences and curls of the field completely determine them}.
...
\item Combining this with the divergence theorem), we know \fbox{$\nabla \cdot \vec{E} = \frac{\rho}{\epsilon_0}$} for the charge density $\rho(\vec{x})$.
\end{itemize}
...
\item All this together means $\vec{E} = -\nabla \vec{V} \Rightarrow$ \fbox{$\nabla^2\vec{V} = -\frac{\rho}{\epsilon_0}$}  (Poisson's Equation)
...
ONe proof (Green's Identity): For a smooth function $u$ on $\mathbb{R}^3$, \fbox{$\iiint_R u\nabla^2u + \nabla u \cdot \nabla u d\vec{x} = \iint_{\delta R} u\nabla u \vec{dA}$}
