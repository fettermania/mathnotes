...
\section{3.3: Surface Integrals}
\begin{itemize}
\item Flux is a specific form of the general $\iint_S F da$.
\item dA is a patch of a parallelogram on the surface.  This is defined by corners $\overrightarrow{x}(u_0, v_0), \overrightarrow{x}(u_0, v_0) + \delta_u \overrightarrow{x}(u_0, v_0)$, and $\overrightarrow{x}(u_0, v_0) + \delta_v \overrightarrow{x}(u_0, v_0)$
\item Therefore, using the parallelogram area formula,$dA = \Delta_u \Delta_v \| \overrightarrow{x}_u \times \overrightarrow{x}_v \|$
\item Taking to the limit, this means the area is $\iint_D F(\overrightarrow{x}(u,v)) \|\overrightarrow{x}_u \times \overrightarrow{x}_v \| du dv$
\end{itemize}
...
\item $d\overrightarrow{x}/du = \langle -r \sin (u) \sin (v),  -r \sin (u) \cos (v), r \cos (u)\rangle$
\item $d\overrightarrow{x}/dv = \langle Rcos(v) + r\cos(u)\cos(v), -R\sin(v) -r\cos(v)\sin(v), 0\rangle$
\item After lots of math, $\| d\overrightarrow{x}/du \times \overrightarrow{x}/dv \| = r(R+ r\cos(u))$
...
\begin{itemize}
\item Center of mass for density $\rho$ is $\frac{\iint_S \overrightarrow{x} \rho dA}{\iint_S \rho dA}$
...
\item $\overrightarrow{x}_{\theta} \times \overrightarrow{x}_{\phi} = \langle \sin^2(\phi) \cos(\theta), \sin^2(\phi)\sin(\theta), \sin(\phi)\cos(\phi) \rangle$
\item Pull out the $\sin(\phi)$ and the remaining norm is one, so $\| \overrightarrow{x}_{\theta} \times \overrightarrow{x}_{\phi}\| = \sin(\phi)$ 
...
\item Object to spin: helicoid $\overrightarrow{x}(u,v) = \langle u \cos (v), u \sin (v), v\rangle u \in [0,R], v \in [0, 2\pi]$
\item Assumption for the problem: $\int_{u=0}^{u=R} u^2 \sqrt(1 +u^2)du = 2$
\item Center of mass for density $\rho$ is $\frac{\iint_S \overrightarrow{x} \rho dA}{\iint_S \rho dA}$
\item Use polar coordinates $r, \theta$.
\item After computation, $\| \overrightarrow{x}_r  \times \overrightarrow{x}_{\theta} \| = \sqrt{1+r^2}$
...
\begin{itemize}
\item Formula: $\Phi = \iint_S (\overrightarrow{E} \cdot \overrightarrow{n}) dA = \iint_S FdA$
\item Field: $\overrightarrow{E} = \langle yz, xz, xy \rangle$
...
\item As usual, $dA = \| \overrightarrow{x}_u \times \overrightarrow{x}_v \| = \sin(v)$.
\item Norm just points out from the center: $\hat{n} = \langle  \cos(u)\sin(v), \sin(u)\sin(v), \cos(v) \rangle $ 
\item $\overrightarrow{E} = \langle \sin(u)\sin(v)\cos(v), \cos(u)\sin(v)\cos(v), \cos(u)\sin^2(v)\sin(u)\rangle$ 
\item So $\overrightarrow{E} \cdot \hat{n} = 3\cos(u)\sin(u)\cos(v)\sin^2(v)$
...
Example: Field $\overrightarrow{E} = \ln(x^2 + y^2)\langle x, y, 0\rangle$ through $R$-wide cylinder, height $h$
\begin{itemize}
\item Parameterize: $x = r \cos \theta, y = r \sin \theta, z = z$
\item \textbf{Top/Bottom}: $\hat{n} = \langle 0, 0, 1 \rangle, \overrightarrow{E} = f(x,y) \langle x, y, 0\rangle \rightarrow \hat{n} \cdot \overrightarrow{E} = 0$
...
Example: Field $\overrightarrow{E} = e^{-x^2-y^2-z^2}\overrightarrow{x}$ with sphere S at radius R, setting $\epsilon_0 = 1$
\begin{itemize}
...
\item $\overrightarrow{x} = R \hat{n}$, so $\overrightarrow{E} \cdot {n} = R e^{-R^2}$
\item $R \iint_{sphere} e^{-R^2} = 4 \pi R^3 e^{-R^2}$
\end{itemize}

\textbf{Note}: In the future we write $\hat{n}dA = \overrightarrow{dA}$
\end{document}
