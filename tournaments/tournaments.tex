...
\fbox{\textbf{Theorem}: The determinant of an $X_n$ with generating coefficients $x_0, x_1 ... x_n$ is $\prod_{0 \leq i < j \leq n}(x_j-x_i)$.}
...
\fbox{If $\det(X_n) = \prod_{0 \leq i < j \leq n}(x_j-x_i)$ for all $X_n$,  then $\det(X_{n+1}) = \prod_{0 \leq i < j \leq n + 1}(x_j-x_i)$ for all $X_{n+1}$}
