...
\item Somehow , this is the function from which all initial setups of the heat equation (specified as $g(x,y)$) are built. 
\end{itemize}
...
\item Therefore our bell curve looks like $p(x,t) = C(t)\exp\{-\frac{x^2}{\sigma^2 t}\}$ 
...
\item Even though our guess was unsubstantiated, we see $p(x,t) = \frac{1}{\sigma \sqrt{\pi t}} \exp\{-\frac{x^2}{\sigma^2 t}\}$ solves $p_t = Dp_{xx}$.  
...
\begin{itemize}
\item A \textbf{convolution} $(\Phi \star g)(x, t)$ of (probability function) $\Phi$ and initial state $g$ is the function that combines the two at point $x$, or $(f \star g)(x) \int_{y=-\infty}^{y=\infty} f(x-y)g(y)dy =  \int_{y=-\infty}^{y=\infty} f(y)g(x-y)dy$.  So it's the probability that $f$ times $g$ is $x$ over the full domain for both.
\item In this case, ($\Phi \star g) = p(x,t) =
 \int_{y=-\infty}^{y=\infty} \Phi(x-y)g(y)dy
 \int_{y=-\infty}^{y=\infty} \frac{1}{\sqrt{4\pi D t}} \exp\{-\frac{1}{4D}\frac{(x-y)^2}{t}\}g(y)dy, t >0$
...
\item Taking derivates $\frac{\delta}{\delta t}, \frac{\delta^2}{\delta x_i^2}$ lets us use $u_t = u_{x_0x_0} + u_{x_1x_1} + ...$
\item Calling the argument $z = \frac{\|x\|^2}{t}$, we take derivatives and find $0 = av(z) + (z+2n)v'(z) + 4zv''(z)$.  Note that $n$ is from $\mathbb{R}^n$, since $\nabla (\|\vec{x}\|^2) = 2\vec{x}$ and $\nabla \cdot (2\vec{x}) = 2n$.
\item How do we choose $a$ so that the equation can be solved as $\frac{1}{4}v(z) + v'(z) = 0$?  
...
\item So the \fbox{fundamental heat solution is $\Phi(\vec{x},t) = (4\pi t)^{-\frac{n}{2}}e^{-\frac{\|x\|^2}{4t}}$}
\end{itemize}
...
\item The transfer at the ``skin'' $\delta G$ is zero so on the boundary, for normal $\hat{n}, D_{\hat{n}} |_{\delta G} = \nabla u \cdot \hat{n} |_{\delta G} = 0$.  Otherwise, the heat transfer, say, would be nonzero.   (Note that this is an example of a Neumann condition.)
...
We can prove a few things, like the fact that the cost decreases over time:
...
\item The second term $-\iiint_G ( u_t \nabla \cdot \nabla u )dx = -\iiint_G ( u_t \nabla^2 u )dx = -\iiint_G [\nabla^2 u]^2 dx$ by definition of heat equation.  The integrand (a square) must be non-negative, so the whole thing has to decrease or stay constant.
\end{itemize}

Note that if $\iiint_G [\nabla^2 u]^2 dx = 0$ (exact equality), then $u$ must be constant.
...
\begin{itemize}
\item Note that $u_{xx} = -\pi^2 (u-u_0), u_{yy} =  -4\pi^2 (u-u_0), u_yy =  -9\pi^2 (u-u_0)$, so $\nabla ^2 u = -14\pi^2 (u-u_0)$
...
\begin{itemize}
\item \textbf{finite difference method}: Discretize the space and use the spherical (surrounding points) average property of harmonic functions.
...
\item And by integration by parts, with $U = e^{-i\omega x}, V = u_{xx}$, we have 
	$= \frac{1}{2\pi}[  e^{-i\omega x}u_x|_{-\infty}^{\infty} - (-i\omega)   \int_{\mathbb{R}} u_{x}(x,t)e^{-i\omega x}dx]$.  The first term is zero from the  boundary conditions ( I guess if $u \rightarrow 0$ then $u_x \rightarrow 0$ too?  I suppose we can assume this for non-insane initial conditions)
...
\item WIth the boundary conditions $\hat{u}(\omega, 0) = \hat{g}(\omega)$ (the transform of the initial $g(x)$ state on $u$, and similarly $\hat{u}(\omega, t) = 0$ implies a solution of $\hat{u}(\omega, t) = \hat{g}(\omega)\cos(|\omega|t)$.  TODO: Why the absolute value?  (especially for an even function like cosine)
\end{itemize}
...
The wave solution then has a Fourier transform $\hat{u}(\omega, t) = \hat{g}(\omega) \cos(|\omega |t)$.  With the initial condition set as, say, $g(x) = 2\pi u_0e^{-\frac{x^2}{2}}$, we can transform $g(x)$ to $\hat{g}(\omega)$, then transform the whole thing back.
\begin{itemize}
...
\item Then, since the combined Fourier-side product is $\hat{u}(\omega, t) = u_0\sqrt{2\pi}e^{-\frac{\omega^2}{2}} \cos(|\omega |t)$, we can transform back with $u(x,t) = \int_{\mathbb{R}} \hat{u}(\omega, t)e^{ix\omega}d\omega$, knowing $\cos(|\omega|t) = \frac{e^{i|\omega|t} + e^{-|\omega|t}}{2}$
...
\item Apparently this is the same solution as with d'Alembert's formula (the two sensors going out from x at time 0 and querying at t):\fbox{ $u(x,t) = \frac{1}{2}[g(x+t) - g(x-t)]$,} since $g(x) = 2\pi u_0e^{-\frac{x^2}{2}}$.
\end{itemize}
...
\item Then $\widehat{u_x}(\omega, t) = \frac{1}{2\pi} \int_{x = -\infty}^{x = \infty} u_x(x,t)e^{-i\omega x} dx = \frac{1}{2\pi} u(x,t) e^{-i \omega x}|_{x = -\infty}^{x = \infty} - (-i\omega) \int_{x = -\infty}^{x = \infty} u(x,y)e^{-i \omega x} dx$ by integration by parts.  Assuming $u \rightarrow 0$ when approaching either infinity, the first term is zero, and the second is just $\hat{u}$, so under standard FT assumptions,
\\
...
This \fbox{$F(x,t;a) =  \sqrt{\frac{1}{4\pi t}}  [e^-\frac{[x-a]^2}{4t}]$} turns out to be a \emph{fundamental solution of the 1D Heat equation}, in that if we have any initial ``data'' $g(x) = u(x,0)$, then we can sum over all points $a$ where the unit of heat is found: 
\\
...
\begin{itemize}
\item $\widehat{{u}_{yy} }  = \frac{1}{2\pi} \int_{x=-\infty}^{x=\infty} u_{yy}e^{-i\omega x} dw$
\item $\widehat{{u}_{yy}} = -\frac{1}{2\pi} \int_{x=-\infty}^{x=\infty} u_{xx}e^{-i\omega x} dw$ by $u_{xx} + u_{yy} = 0$
...
\item So, we have combinations of two possible solutions for the transform of $u$, where $u_{xx}+u_{yy}=0$: $A(\omega)e^{|\omega|y} + B(\omega)e^{-|\omega|y} $
...
Results:
\begin{itemize} 
\item \fbox{1D waves ($u(x,t) = \frac{g(x+t)+g(x-t)}{2})$}
\item \fbox{3D (compression) waves ($u(\vec{x}, t)= \frac{u_0}{2\|x\|}[(\| x\| + t)\exp\{  \frac{(\|x\| + t)^2}{2} \}  + (\| x\| - t)\exp\{  \frac{(\|x\| - t)^2}{2} \}  ]$ }
\end{itemize}
...
This uses the Fourier transform to show an isomorphism between a probability density over position (x) with that over v velocity (p), and shows that the other side of the FT can have intrinsic meaning.
...
\item The transformed ``motion'' ($\hat{u}$) sees $\|\hat{u}(p,t)\|^2$ as probability particle's \emph{velocity vector} is near $\vec{p}$ at time $t$.
\item This works because, for expected position $X = \iiint_x x \|u(x,t)\|^2$, change in expected position is expected change in position is expected velocity: $\frac{dX}{dt}$ = $\iiint_{\vec{p} \in \mathbb{R}^3} \vec{p}(2\pi)^3 |\hat{u}(p,t)|^2 d\vec{p}$ TODO come back here 12/25/22
\end{itemize}
...
\item Knowing $u_t = \frac{i}{2}\nabla^2 u$ means $u_t^* = -\frac{i}{2}\nabla^2 u$. Then $\frac{d}{dt}[\iiint_{\mathbb{R}^3} |u|^2 d\vec{x}] = \frac{d}{dt}[\iiint_{\mathbb{R}^3} uu^* d\vec{x}] = \iiint u_tu^* + uu_t^* = \frac{i}{2} \iiint u^*\nabla^2 u - u\nabla^2 u^* = \nabla \cdot ( u^*\nabla u - u\nabla u^* )$ (Note: This is an identity - take $\nabla(f\nabla g - g\nabla f)$ and see!)
\item So If $u, \nabla u \rightarrow 0$ go to zero near infinity, then we can use divergence theorem (since any ball would be zero on the surface).  So I suppose this means $\iiint_{\mathbb{R}^3} |u|^2 d\vec{x}]$ is constant (and can therefore be adjusted to 1?) Those are the three conditions for using $u(x,0) = g(x)$ as an initial distribution.
\end {itemize}
...
\exp \{-\frac{\pi \|p - p_0\|^2}{2\sigma ^2} \}, \sigma << 1$ , which looks like a ``normal'' about point $p_0$ with some $\sigma^2$ variance:
\begin {itemize}
...
You can prove the angles are the same before and after a harmonic map with $x_u=y_v, x_v=-y_u$ by:
...
Combining all of these maps, including the map that elevates and rotates a unit disk to $(x, 0, z)$ in $D: (x=u, z=1+v)$, maps $(u,v)$ on the unit disk to \fbox{$(x,y,z) = \frac{1}{u^2+(v-1)^2} \langle 4u, 2(1-u^2-v^2), 0\rangle$}, with an inverse of \fbox{$(u,v) = \frac{1}{x^2+(y+2)^2} \langle 4x, x^2+y^2-4\rangle$}
...
\item Method: Writing terms as recurrence relation
\begin{itemize}
...
If we center on 0, $y(0.95)$ converges to 0.001\% error in 8 iterations: ($n = 0, 2, ...14$).
...
\begin{itemize}
\item Separate variables by saying $\psi(x,t) = X(x)T(t)$ and take derivatives.
\item Divide by $XT$: $iXT'= -\frac{1}{2} X''T + U(x)XT \Rightarrow T'/T= \frac{1}{2} iX''/X + U(x)$
\item Somehow (unknown step here)  we end up with $\psi(x,t) = y(x)e^{-iEt}$, where $y(x)$, a renamed $X(x)$, has property $-\frac{1}{2} y''(x)+U(x)y(x) = Ey(x) = -\frac{1}{2}y''(x)+\frac{1}{2}x^2y(x)$
\item Near $x_0$, we can approximate y(x) with:
\begin{itemize}
\item TRICK - turn $x$ into $x-x_0$: $\frac{1}{2}x^2 = \frac{1}{2}([x-x_0] + x_0) = \frac{1}{2}[x-x_0]^2 + x_0[x-x_0] + \frac{1}{2}x_0^2$
...
Weird: we're defining a new variable $t = (2x_0)^{\frac{1}{3}}[x-x_0]$, which isn't time, but where $t>0$ is a forbidden region into which our block tunnels.  Coupled with $y''-ty=0$:
\begin{itemize}
\item Write out terms to see that $y'(t)-ty(t) = \sum_{n=2}^{\infty} a_nn(n-1)t^{n-2} - \sum_{n=0}^{\infty} a_nt^{n+1} =0$.  It looks like the sums skip over two terms at a time!
\item Writing out the first few terms, the sum is $2a_2+\sum_{n=1}^{\infty} [(n+2)(n+1)a_{n+2}-a_{n-1}]t^n = 0$
\item Recurrence writing (base case) method: Since this is always true, try $t=0$ to see that $a_2$ must be 0.
\item Recurrence writing (recursive case) method: Try $t = 0$ to see that $[(n+2)(n+1)a_{n+2}-a_{n-1}] = 0$ or that $a_{n+2} =\frac{a_{n-1}} {(n+2)(n+1)}, n \geq 1$ 
...
Main idea: We had a solution of $a_0y_0(t) + a_1y_1(t)$ for the last problem.  How do we know that $y_0(t)$ and $y_1(t)$ are different?  As in, they are not multiples of each other or really, they are linearly independent?  We use a matrix format for piles of these solutions and check if the determinant is nonzero.  This can be made clear for $n=2$, but it appears subtle for higher orders.
...
\begin{itemize}
\item Starting with the diff eqs $y_0''-ty_0 = 0, y_1''-ty_1 = 0 \Rightarrow y_0''/y_0 = t = y_1''/y $
\item Then $y_0y_1''=y_0''y_1 \Rightarrow y_0y_1'' - y_0''y_1 = 0$ (result 1)
\item Definition of Wronskian determinant $W(t) = y_0y_1'-y_0'y_1$
\item $W'(t) = y_0y_1''+y_0'y_1'-y_0''y_1-y_0'y_1' = y_0y_1'' - y_0''y_1$, which is zero by result 1 above.
\item A zero derivative of $W(t)$ means that $W(t)$ does not depend on $t$.
\item This means that we can use ANY value of $t$ to plug into $W(t)$ for the Wronskian determinant. For the Airy equations try $t=0$: $y_0(0)y_1'(0) - y_1(0)y_0'(0) = (1)(1) - (0)(0) = 1$
\end{itemize}
...
\begin{itemize}
\item $y''+p_1y'+p_2y=0 \Rightarrow p_1y'=-y''-p_2y$ (rearrange)
\item $W = y_0y_1' - y_0'y_1$ (definition of Wronksian)
\item $p_1W = y_0(p_1y_1') - y_1(p_1y_0') = y_0(-y_1''-p_2y_1) - y_1(-y_0''-p_2y_0) = y_1y_0''-y_0y_1''$ (sub in $p_1y'$ from first bullet)
\item This last term is $-W'(t)$ by above results.  
\item So $p_1W = W' \Rightarrow W = c\exp\{-\int p_1(t)dt\}$ by basic homogeneous diff eq. solution.
\end{itemize}

This is \textbf{Abel's formula}, and (somehow) generalizes to solve any 
\\
\fbox{$y^{(n)} + p_1(t)y^{(n-1)} + ... + p_ny(t) = 0$, as $W(t) = \det W[y_0, ... y_{n-1}](t) = c\exp\{-\int p_1(t)dt\}$}

Applying Abel's formula to find solutions to $y''(t) + 2y'(t) + y(t) = 0$
\begin{itemize}
...
\item Even though Cauchy-Euler suggests $t>0$, we suppose the equation needs to work for all $t$ (Is this obvious?), so we know $a_0$.
\item However, since the whole power series is identically zero at all times, this MUST mean all the coefficients are zero.  So this method only produces $y(t)=0$
\end{itemize}
...
\item Finding two linearly indepedent solutons of form $y(t)=t^r$: $W[y_0,y_1](t) =  \begin{pmatrix}
 ...
 \item Case 1: Roots are distinct and real, so we have $y(t) = a_+t^{r_+} + a_-t^{r_-}$
...
\item 
 ...
 \item So combinations of $at^r + bt^{\bar{r}}$ end up being $t^{Re(r)}[c_0\cos(Im(r)\ln(t)) +c_1 \sin(Im(r)\ln(t)]$
\end{itemize}
...
For some reason, in our (Airy?) equation, $r^2R(r)''+rR'(r) -m^2R(r) = 0$, $m=0$, so we have a Cauchy-Euler with $\alpha = 1$ and a solution with repeated root 0.  So our solution ends up being $R(r) = ar^0 + br^0\ln(r) = a+b\ln(r)$
...
\begin{itemize}
\item 1. Leads with $t^2y''(t)$ like the Cauchy-Euler Equation (5.5). Therefore it will include non-integer exponents in the series.  (like $t^{Re(r)}$, maybe? Why?)
...
\item Assume the solution is of the form $y(t) = t^r\sum_{n=0}^{\infty} a_n t^n$.  Note that the general form is $(t-t_0)$ for all those $t$'s but presented often with $t_0 = 0$ for simplicity.
...
Aside; There's an exception here where we can find  a closed-form Bessel function.
\begin{itemize}
...
Main idea: A simple quantum harmonic oscillator, like the Airy equation.  Somehow we will learn \emph{eveything} with Hermite's Equation (Note: this seems like we learned how many solutions and how to use them as a probability measure)
...
Solving this equation is hard, but selecting $E=\frac{1}{2}$ and magically, assuming $b_1 = 0$, we have:
...
Why did we choose $E=\frac{1}{2}$?  Let's find a solution without using a series.
\begin{itemize}
...
The energy level $E=\frac{1}{2}$ sees an oscillating solution, but \textbf{this is not the only solution}.  Many oscillator energies (turns, out, any $E =n +  \frac{1}{2}$) exist, and share the same feature: $x \rightarrow \pm \infty \Rightarrow y \rightarrow 0$.  In other words, they all go to zero at the extremes and therefore ``look like'' the lowest energy ($E=\frac{1}{2}$) solution $a_0e^{-\frac{x^2}{2}}$

This suggests we attempt another solution separating out this factor.  Let's guess $y = z(x)e^{\frac{-x^2}{2}}$, where again $E y(x) = -\frac{1}{2}y''(x) + \frac{1}{2}x^2y(x)$.
...
The upshot of this is that we want to find our probability of the oscillator with energy $E_n = n+\frac{1}{2}$ existing near $x$.  The function $|y_n(x)|^2 = H_n(x)^2e^{-x^2}$ is that density, with $H_0(x)=a_0, H_1(x) = a_1x, H_2(x) = a_2[1-2x^2]$.  

Application: How to find the probability that the $E_0$ oscillator (centered at $x=0$) is outside ``classic bounds'' (turning points)?
...
$|\Phi(\rho, \theta, \omega)|^2$ measures the likelihood of finding an electron a certainn point away from the center of an atom, in spherical coordinates.  The potential energy between the electron and the proton goes down inversely with distance $\rho$ but doesn't depend on the angles. 

 Putting this into Schroedinger's equation in spherical coordinates, collapsing physical constants into $\rho_0$:
...
\item So now $\frac{1}{\rho^2}\frac{\delta}{\delta \rho}(\rho^2 \frac{\delta \Phi}{\delta \rho}) + \frac{1}{\rho^2}L^2(\Phi) + \frac{\rho_0}{\rho}\Phi = \Phi$
\item The main trick to solving $\Phi$: separate variables $\Phi = R(\rho)Y(\theta, \phi)$
...
\begin{itemize}
\item Note that as $\rho \rightarrow \infty$, then $\rho^2$ dominates, so this becomes $\rho^2R''(p) = \rho^2R(r)$.  The solutions are $R(\rho) = e^{\rho}$ or $R(\rho) = e^{-\rho}$.  Only the latter dies off as $\rho \rightarrow \infty$, which makes sense, since the probability $\Phi$ would decrease the further out one gets.  This is our ``long term behavior'' of $R$.
...
\begin{itemize}
\item Each term is $a_{n+1} = \frac{2(1+n+l) -\rho_0}{(n+1)(n+2(l+1))}, n\geq 0$
...
(Note: If we're doing cleanup, can substitute $\varphi$ for $\phi$, but whatever)
...
\begin{itemize}
\item We know from the last section that $\frac{L^2(Y(\theta, \phi))}{Y(\theta, \phi)} 
...
\item (Note that, separating the ever increasing $n$ part of the numerator from the $l$ part, the dominant part of this is basically a harmonic series so it doesn't converge anywhere)
