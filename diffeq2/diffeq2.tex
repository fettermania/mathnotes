...
\item Set $r=r_1$ and solve the whole thing for one solution, and same for $r=r_2$.  if they're the same, look to \textbf{Airy's equation} to find a linearly independent partner.
...
Final case: If $\nu = 0$, we need the OTHER solution beyond case 1's series solution.  We'll use the Wronksian (with our known solution) with Airy's equation (for our $p_1$ in our initial diff eq):
\begin{itemize}
\item The determinant can be found with Airy's equation: $W(t) = c \exp \{-\int p_1(t) dt\}$ for $t^2y''(t) + ty'(t) + t^2y(t) = 0$
