...
We assert without proof that $M$ is aperiodic (we're not going to get into a cycle), so by Markov chain theory, repeated application of $M$ will yield the equilibrium behavior no matter the initial state distribution $\vec{a}$, as long as the sum of the vector's values are 1.  Therefore, we need to find the equilibrium solution as shown in Fig.~\ref{fig:equilibrium}.
