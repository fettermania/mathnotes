...
\end{scope}
\end{tikzpicture}
\caption{$n = s(g-1)+1$. Here, $s=4, g=4$}
...
Suppose that that every card has $s$ symbols, and any symbol has exactly $g$ cards containing it\footnote{for example, all cards contain $s=4$ symbols in Fig.~\ref{fig:s4g3}, which each appear on $g=3$ cards}. Then 
...
\emph{Algorithm Summary}: Our algorithm takes every constant $c$ in a finite field $\mathcal{F}$, and goes through the multiplication table of $\mathcal{F}$ row ($y$) by row. To a clique $C_{c,y}$ specified by these two, we add an element from $G_x$ for every column $x$ : The entry from $G_x$ indexed by the entry at $z = (x,y)$ in the table, plus the constant $c$.
