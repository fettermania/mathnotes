...
\end{scope}
\end{tikzpicture}
\caption{s=5, g=5 from s=5, g=4 with no edges. The green is an example $I_1$ clique.}
\label{fig:s5g5}
\end{figure}
...
\begin{itemize}
\item If $g=s$, then $n=g^2-g+1$ by the Graph Representation Theorem and $n=m$ by the Candidate Theorem. Thus, we're looking for a PDS of size $g^2-g$.
\end{itemize}
...
\textyf{An important consequence}: Because $g=s$ in both the cycle generation in section 3 as well as the PDS generation in section 5, these are, up to isomorphism, the same coloring.
